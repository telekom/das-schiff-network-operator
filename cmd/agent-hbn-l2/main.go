...
import (
	...
	controllerhbnl2 "github.com/telekom/das-schiff-network-operator/controllers/agent-hbn-l2"
	reconcilerhbnl2 "github.com/telekom/das-schiff-network-operator/pkg/reconciler/agent-hbn-l2"
...
func setupReconcilers(mgr manager.Manager) (*reconcilerhbnl2.NodeNetplanConfigReconciler, error) {
	r, err := reconcilerhbnl2.NewNodeNetplanConfigReconciler(mgr.GetClient(), mgr.GetLogger())
	if err != nil {
		return nil, fmt.Errorf("unable to create debounced reconciler: %w", err)
	}

	if err = (&controllerhbnl2.NodeNetplanConfigReconciler{
