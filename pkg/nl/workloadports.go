...
// an attachment goes away the CNI normally removes the veth, which takes its
// addresses and on-link routes with it; a workload port that is still present
// without an attachment (the entry was dropped or deleted while the CNI
// teardown is pending or failed) is stripped of that state and released from
// its VRF instead, so its host routes cannot stay eligible for export.
//
...
func (n *Manager) ReconcileWorkloadPorts(cfg *NetlinkConfiguration) error {
	// An entry that fails to program stays in the desired set: its own
	// reconcile is authoritative for the port's state and is retried, while
	// stripping the port in the meantime would only make it flap.
	desired := make(map[string]bool, len(cfg.WorkloadPorts))
	var errs []error
	for i := range cfg.WorkloadPorts {
		desired[cfg.WorkloadPorts[i].Interface] = true
	...
	if err := n.cleanupWorkloadPorts(desired); err != nil {
		errs = append(errs, err)
	}
	return errors.Join(errs...)
}

// cleanupWorkloadPorts strips the routed state from workload ports that no
// attachment wants (any more): the CNI's veths — identified by the ifalias, the
// same test that gates programming — that are not in desired lose the gateway
// addresses and RTPROT_BOOT host routes this reconciler installs and are
// released from their VRF. Only a VRF master is released: nothing here ever
// enslaves a port to anything else, so another master is not this pass's to
// touch. The port itself is left in place for the CNI to remove.
//
// This is safe against an ADD in flight: the CNI creates the veth and stamps
// the alias before the agent records the entry, but it programs nothing on the
// CRA side itself, so a port seen before its entry has nothing to strip.
func (n *Manager) cleanupWorkloadPorts(desired map[string]bool) error {
	...
	for _, link := range links {
		...
		if desired[name] || !isWorkloadPortLink(link, name) {
			continue
		}
		stale := &WorkloadPort{Interface: name}
		if err := n.reconcileWorkloadPortAddrs(link, stale); err != nil {
			errs = append(errs, fmt.Errorf("error releasing stale workload port %q: %w", name, err))
			continue
		}
		if err := n.reconcileWorkloadPortRoutes(link, stale, unix.RT_TABLE_MAIN); err != nil {
			errs = append(errs, fmt.Errorf("error releasing stale workload port %q: %w", name, err))
			continue
		}
		if master, ok := byIndex[link.Attrs().MasterIndex]; ok && master.Type() == linkTypeVRF {
			if err := n.toolkit.LinkSetNoMaster(link); err != nil {
				errs = append(errs, fmt.Errorf("error releasing stale workload port %q from VRF %q: %w",
					name, master.Attrs().Name, err))
			}
		}
	}
	return errors.Join(errs...)
...
func (n *Manager) ReconcileL2AttachedPorts(cfg *NetlinkConfiguration) error {
	...
	//
	// Trunk members are programmed one by one on purpose: the all-or-nothing
	// contract of an attachment holds at resolution time (the agent records
	// the entry only if every referenced Layer2 resolves), whereas here every
	// member is equally authorised by that entry, so a member whose bridge
	// cannot be programmed right now is no reason to cut the workload off the
	// domains that can. Tearing the others down would only make them flap
...
// programming — and only ever an 802.1Q one, so exactly those links are ours
// to remove (an 802.1ad link somebody else stacked on the port under the same
// name is not, just as ensureL2TrunkSubinterface refuses to adopt it). That
// also covers
...
func (n *Manager) cleanupL2TrunkSubinterfaces(desired map[string]map[uint16]bool,
	...
	for _, link := range links {
		...
		// Only ever remove interfaces this reconciler could have created: an
		// 802.1Q vlan with the name it derives, on a workload port.
		if vlan.VlanProtocol != netlink.VLAN_PROTOCOL_8021Q ||
			vlan.Name != l2TrunkSubinterfaceName(parent.Attrs().Name, vlanID) ||
