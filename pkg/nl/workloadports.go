...
// ReconcileL2AttachedPorts enslaves the workload-CNI L2 attach ports (moved into
// this netns by the CNI) to their Layer2 bridge (l2.<vlanID>) as bridge slaves
// with no L3 addressing. Like ReconcileWorkloadPorts it is adopt-only: a port
// that is not present yet (or already gone) is skipped.
