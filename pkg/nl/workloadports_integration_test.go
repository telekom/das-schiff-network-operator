...
// TestReconcileWorkloadPortsReleasesOrphanedPort ensures a workload port that
// outlived its attachment — the entry is gone, the CNI's teardown of the veth
// has not happened (yet) — is stripped of its gateway addresses and host
// routes and released from its VRF, so nothing on it stays eligible for
// export. A veth with the same state but without the CNI's alias is not the
// reconciler's and is left alone.
func TestReconcileWorkloadPortsReleasesOrphanedPort(t *testing.T) {
	requireRoot(t)
	...
	const (
		orphan  = "craorphan0001"
		foreign = "crastranger02"
	)
	var vrf *netlink.Vrf
	if derr := testNS.Do(func(_ ns.NetNS) error {
		vrf = &netlink.Vrf{LinkAttrs: netlink.LinkAttrs{Name: "tenant"}, Table: routedTestVRFTable}
		...
		if e := addWorkloadVethPort(orphan); e != nil {
			return e
		}
		return addVethPort(foreign)
	}); derr != nil {
		t.Fatalf("populate netns: %v", derr)
	}

	mgr := NewManager(&Toolkit{}, nil)
	entry := &NetlinkConfiguration{WorkloadPorts: []WorkloadPort{{
		Interface: orphan, VRF: "tenant", GatewayV4: "169.254.1.1/32", GatewayV6: "fe80::1/128",
		HostRoutes: []string{"10.100.0.7/32", "fd00:100::7/128"},
	}}}
	if derr := testNS.Do(func(_ ns.NetNS) error {
		if e := mgr.ReconcileWorkloadPorts(entry); e != nil {
			return e
		}
		if e := plantForeignPortState(foreign, vrf); e != nil {
			return e
		}
		// The attachment is gone; the port is not.
		return mgr.ReconcileWorkloadPorts(&NetlinkConfiguration{})
	}); derr != nil {
		t.Fatalf("ReconcileWorkloadPorts: %v", derr)
	}

	if derr := testNS.Do(func(_ ns.NetNS) error {
		if e := assertPortStripped(t, orphan); e != nil {
			return e
		}
		link, e := netlink.LinkByName(foreign)
		if e != nil {
			return fmt.Errorf("looking up foreign veth: %w", e)
		}
		if link.Attrs().MasterIndex == 0 {
			t.Error("foreign veth was released from its VRF")
		}
		addrs, _ := netlink.AddrList(link, netlink.FAMILY_V4)
		if len(addrs) != 1 {
			t.Errorf("foreign veth addresses = %v, want its one address kept", addrs)
		}
		assertHostRoutes(t, routedTestVRFTable, "10.100.0.8/32", "")
		return nil
	}); derr != nil {
		t.Fatalf("netns check: %v", derr)
	}
...
// plantForeignPortState gives the veth name the same kind of state the
// reconciler installs on a workload port — VRF membership, a gateway address,
// an on-link host route — as if somebody else had put it there.
func plantForeignPortState(name string, vrf *netlink.Vrf) error {
	link, err := netlink.LinkByName(name)
	if err != nil {
		return fmt.Errorf("looking up foreign veth: %w", err)
	}
	if err := netlink.LinkSetUp(link); err != nil {
		return fmt.Errorf("setting foreign veth up: %w", err)
	}
	if err := netlink.LinkSetMaster(link, vrf); err != nil {
		return fmt.Errorf("enslaving foreign veth: %w", err)
	}
	addr, _ := netlink.ParseAddr("169.254.1.1/32")
	if err := netlink.AddrAdd(link, addr); err != nil {
		return fmt.Errorf("adding foreign address: %w", err)
	}
	_, dst, _ := net.ParseCIDR("10.100.0.8/32")
	if err := netlink.RouteAdd(&netlink.Route{LinkIndex: link.Attrs().Index, Dst: dst,
		Scope: netlink.SCOPE_LINK, Table: routedTestVRFTable, Protocol: unix.RTPROT_BOOT}); err != nil {
		return fmt.Errorf("adding foreign route: %w", err)
	}
	return nil
}

// assertPortStripped checks that the workload port name carries none of the
// reconciler's state any more: no VRF master, no single-address (gateway)
// address and no RTPROT_BOOT route in any table.
func assertPortStripped(t *testing.T, name string) error {
	t.Helper()
	link, err := netlink.LinkByName(name)
	if err != nil {
		return fmt.Errorf("looking up port %q: %w", name, err)
	}
	if link.Attrs().MasterIndex != 0 {
		t.Errorf("port %q still enslaved (master=%d)", name, link.Attrs().MasterIndex)
	}
	addrs, _ := netlink.AddrList(link, netlink.FAMILY_ALL)
	for i := range addrs {
		if ones, bits := addrs[i].Mask.Size(); ones == bits {
			t.Errorf("port %q still carries gateway address %s", name, addrs[i].IPNet)
		}
	}
	routes, _ := netlink.RouteListFiltered(netlink.FAMILY_ALL,
	...
	for i := range routes {
		if routes[i].Protocol == unix.RTPROT_BOOT {
			t.Errorf("port %q still carries host route %s in table %d", name, routes[i].Dst, routes[i].Table)
		}
	}
	return nil
}
...
func TestReconcileL2TrunkRemovesOrphanedSubinterfaces(t *testing.T) {
	...
	if derr := testNS.Do(func(_ ns.NetNS) error {
		...
		if e := netlink.LinkAdd(&netlink.Vlan{LinkAttrs: attrs, VlanId: 100}); e != nil {
			return fmt.Errorf("adding %s.100: %w", foreign, e)
		}
		// An 802.1ad link on the workload port itself, carrying the very name
		// this reconciler would derive: not ours either, only 802.1Q is.
		pLink, e := netlink.LinkByName(port)
		if e != nil {
			return fmt.Errorf("looking up %s: %w", port, e)
		}
		ad := netlink.NewLinkAttrs()
		ad.Name = port + ".200"
		ad.ParentIndex = pLink.Attrs().Index
		return netlink.LinkAdd(&netlink.Vlan{LinkAttrs: ad, VlanId: 200, VlanProtocol: netlink.VLAN_PROTOCOL_8021AD})
	}); derr != nil {
		t.Fatalf("populate netns: %v", derr)
	}
	...
	if derr := testNS.Do(func(_ ns.NetNS) error {
		...
		if _, e := netlink.LinkByName(port + ".200"); e != nil {
			t.Errorf("802.1ad link %s.200 on the workload port was removed: %v", port, e)
		}
		return nil
	}); derr != nil {
		t.Fatalf("netns check: %v", derr)
