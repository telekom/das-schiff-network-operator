...
// SocketPathEnv is the environment variable that overrides DefaultSocketPath on
// the agent side. The plugin's counterpart is the agentSocket key of its node
// config; both are node-level settings the operator has to change together.
const SocketPathEnv = "WORKLOAD_CNI_SOCKET"
...
func resolveLayer2Members(spec *v1alpha1.NodeNetworkConfigSpec,
	...
	if e.Layer2AttachmentRef != nil {
		name, err := findLayer2(spec, e.Layer2AttachmentRef)
		if err != nil {
			return nil, err
		}
...
// ref. The schema does not make the stamped references unique across
// spec.layer2s, so a reference carried by more than one Layer2 is reported as
// ambiguous rather than resolved to whichever entry map iteration yields first:
// the workload would otherwise land on a nondeterministic bridge.
func findLayer2(spec *v1alpha1.NodeNetworkConfigSpec, ref *v1alpha1.Layer2AttachmentRef) (string, error) {
	var found []string
	...
	switch len(found) {
	case 0:
		return "", fmt.Errorf("Layer2Attachment %s is not configured on this node", layer2AttachmentRefLog(ref))
	case 1:
		return found[0], nil
	default:
		slices.Sort(found)
		return "", fmt.Errorf("Layer2Attachment %s is ambiguous on this node: stamped on Layer2s %s",
			layer2AttachmentRefLog(ref), strings.Join(found, ", "))
	}
}
