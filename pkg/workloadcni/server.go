...
// checkLayer2Preconditions refuses an L2 attachment whose Layer2Attachment
// references cannot be resolved against the node's current NodeNetworkConfig.
...
func (s *Server) checkLayer2Preconditions(ctx context.Context, entry *v1alpha1.WorkloadPortEntry) error {
	...
	if _, err := resolveLayer2Members(&cfg.Spec, entry); err != nil {
		return status.Errorf(codes.FailedPrecondition,
