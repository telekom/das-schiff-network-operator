...
const (
	...
	socketPerm = 0o600
	// staleSocketProbeTimeout bounds the connect attempt that tells a live
	// listener apart from a socket file left behind by a previous run.
	staleSocketProbeTimeout = time.Second
...
// removeStaleSocket unlinks a socket file left behind by a previous run so
// Listen can bind. Only ever an actual socket is removed: the path is
// operator-supplied, and blindly removing it would delete whatever it happens
// to point at (Lstat, not Stat, so a symlink is reported as such and refused
// rather than followed). And only a dead one: a socket that still accepts
// connections belongs to an agent that is still serving — an overlapping
// restart, or a second instance pointed at the same path — and unlinking it
// would leave two servers mutating NodeWorkloadPorts independently, so that is
// refused too.
func removeStaleSocket(ctx context.Context, socketPath string) error {
	...
	if err != nil {
		return fmt.Errorf("inspecting socket path %s: %w", socketPath, err)
	}
	...
	if err == nil {
		_ = conn.Close()
		return fmt.Errorf("refusing to remove %s: another server is still listening on it", socketPath)
	}
...
func (s *Server) Serve(ctx context.Context, socketPath string) error {
	...
	if err := removeStaleSocket(ctx, socketPath); err != nil {
		return err
	}
...
// reference that does not resolve, an MTU no referenced domain can carry, or a
// config the agent has rejected as invalid. The merge would otherwise record
// the entry and silently skip it on every reconcile, leaving the pod running
// with a dead port; failing the ADD instead makes the runtime retry the sandbox
// until the L2A pipeline has stamped the domain onto the node (or the operator
// notices the FailedPrecondition).
//
// The references are resolved against the spec, which is also what the merge
// resolves them against. A spec revision that is still rolling out is not a
// reason to refuse: the port is attached by the very reconcile that realises
// the revision (the bridges are created before the attach pass), so it comes
// up together with it. A revision the agent has already tried and marked
// invalid will not be realised, though, so an attachment into it is refused.
//
...
func (s *Server) checkLayer2Preconditions(ctx context.Context, entry *v1alpha1.WorkloadPortEntry) error {
	...
	if cfg.Status.ConfigStatus == nodeConfigStatusInvalid && cfg.Status.LastAppliedRevision == cfg.Spec.Revision {
		return status.Errorf(codes.FailedPrecondition,
			"NodeNetworkConfig %s revision %s is invalid and will not be applied (%s); its Layer2 domains are not available",
			s.nodeName, cfg.Spec.Revision, cfg.Status.ErrorMessage)
	}
...
// nodeConfigStatusInvalid is the NodeNetworkConfig ConfigStatus the agent sets
// when it failed to apply the current revision (pkg/reconciler/operator's
// StatusInvalid; the package is not imported to keep this transport package
// free of the operator's dependencies).
const nodeConfigStatusInvalid = "invalid"
...
// MinPortMTU is the smallest MTU a workload port may request. It matches the
// Layer2 / Layer2Attachment API minimum so every valid L2 domain can be
// attached; the IPv6 minimum of 1280 is enforced by the plugin only when the
// IPAM result actually carries an IPv6 address.
const MinPortMTU = 1000
...
// Del removes a workload attachment (idempotent).
//
//nolint:wrapcheck // gRPC status errors are the wire representation and must be returned verbatim.
