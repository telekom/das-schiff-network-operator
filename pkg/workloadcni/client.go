...
// dial connects to the node-local workload-cni gRPC socket. An empty socketPath
...
func dial(socketPath string) (*grpc.ClientConn, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("dial workload-cni socket %s: %w", socketPath, err)
	}
	return conn, nil
}
...
func Add(ctx context.Context, socketPath string, req *pb.AddRequest) error {
	...
	if _, err := pb.NewWorkloadCNIClient(conn).Add(ctx, req); err != nil {
		return fmt.Errorf("workload-cni Add: %w", err)
	}
	return nil
}
...
func Del(ctx context.Context, socketPath string, req *pb.DelRequest) error {
	...
	if _, err := pb.NewWorkloadCNIClient(conn).Del(ctx, req); err != nil {
		return fmt.Errorf("workload-cni Del: %w", err)
	}
	return nil
}
