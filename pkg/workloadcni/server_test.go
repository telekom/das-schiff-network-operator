...
// socketTempDir is a short-lived directory for unix sockets. Not t.TempDir():
// unix socket paths are limited to ~104 bytes on some platforms and the
// per-test directory name alone can exceed that.
...
func TestServeRefusesLiveSocket(t *testing.T) {
	sock := filepath.Join(socketTempDir(t), "w.sock")
	// Another agent (an overlapping restart, say) is still serving here.
	var lc net.ListenConfig
	other, err := lc.Listen(context.Background(), "unix", sock)
	if err != nil {
	...
	if err := s.Serve(ctx, sock); err == nil {
		t.Fatal("expected Serve to refuse a socket another server is listening on")
	}
	// The other server must still be reachable: its socket was not unlinked.
...
func TestServeReplacesStaleSocket(t *testing.T) {
	sock := filepath.Join(socketTempDir(t), "w.sock")
	// A socket file whose server is gone: closing without unlinking is what a
	// crashed agent leaves behind.
	var lc net.ListenConfig
	stale, err := lc.Listen(context.Background(), "unix", sock)
	if err != nil {
		t.Fatalf("listen: %v", err)
	}
	...
	if err != nil {
		cancel()
		t.Fatalf("new server did not come up on the stale path: %v (serve: %v)", err, <-done)
	}
	_ = conn.Close()
	cancel()
	if err := <-done; err != nil {
...
func TestServeReturnsNilWhenContextAlreadyCancelled(t *testing.T) {
	...
	s := NewServer(newFakeClient(t), "node-1", logr.Discard())
	if err := s.Serve(ctx, filepath.Join(socketTempDir(t), "w.sock")); err != nil {
		t.Fatalf("expected a clean shutdown, got %v", err)
	}
}
...
func TestServerAddRejectsUnstampedLayer2(t *testing.T) {
	...
	t.Run("invalid NodeNetworkConfig at the current revision", func(t *testing.T) {
		c := newFakeClient(t)
		blue := v1alpha1.Layer2AttachmentRef{Name: "blue", Namespace: DefaultLayer2Namespace}
		cfg := &v1alpha1.NodeNetworkConfig{}
		cfg.Name = "node-1"
		cfg.Spec.Revision = "rev-1"
		cfg.Spec.Layer2s = map[string]v1alpha1.Layer2{"l2.500": {VLAN: 500, AttachmentRef: &blue}}
		cfg.Status = v1alpha1.NodeNetworkConfigStatus{
			ConfigStatus: nodeConfigStatusInvalid, LastAppliedRevision: "rev-1", ErrorMessage: "vlan clash",
		}
		if err := c.Create(ctx, cfg); err != nil {
			t.Fatal(err)
		}
		...
		if msg := status.Convert(err).Message(); !strings.Contains(msg, "invalid") || !strings.Contains(msg, "vlan clash") {
			t.Fatalf("error should report the invalid config and its reason, got %q", msg)
		}

		// A newer revision that is still rolling out is not held against the
		// attachment: it is attached by the reconcile that realises it.
		cfg.Spec.Revision = "rev-2"
		if err := c.Update(ctx, cfg); err != nil {
			t.Fatal(err)
		}
		if _, err := NewServer(c, "node-1", logr.Discard()).Add(ctx, access); err != nil {
			t.Fatalf("Add against a pending revision: %v", err)
		}
	})
