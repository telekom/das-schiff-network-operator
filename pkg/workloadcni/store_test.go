...
// TestMergeL2RejectsAmbiguousReference covers a stamped reference that two
// Layer2s of the node carry: the schema does not forbid it, and picking one by
// map order would bridge the workload into a nondeterministic domain, so the
// entry is dropped and the ambiguity named.
func TestMergeL2RejectsAmbiguousReference(t *testing.T) {
	cfg := trunkCfg()
	cfg.Spec.Layer2s["l2.300"] = v1alpha1.Layer2{
		VNI: 300, VLAN: 300,
		AttachmentRef: &v1alpha1.Layer2AttachmentRef{Name: "green", Namespace: "tenant-a"},
	}
	_, err := resolveLayer2Members(&cfg.Spec, trunkEntry("c1", "cra-trunk", []string{"green"}, []*uint16{nil}))
	if err == nil || !strings.Contains(err.Error(), "ambiguous") ||
		!strings.Contains(err.Error(), "l2.100, l2.300") {
		t.Fatalf("expected an ambiguity error naming both Layer2s, got %v", err)
	}
	if MergeIntoNodeNetworkConfig(cfg, []v1alpha1.WorkloadPortEntry{
		*trunkEntry("c1", "cra-trunk", []string{"green"}, []*uint16{nil}),
	}, logr.Discard()) {
		t.Fatal("expected merge to drop the entry")
	}
	for name, l2 := range cfg.Spec.Layer2s {
		if len(l2.AttachedPorts) != 0 {
			t.Fatalf("expected no ports on %s from an ambiguous reference, got %+v", name, l2.AttachedPorts)
		}
	}
}
