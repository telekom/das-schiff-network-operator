...
var (
	...
	processProvisioning = &v1alpha1.NodeConfigProcess{
		ObjectMeta: metav1.ObjectMeta{
			Name: processName,
		},
		Spec: v1alpha1.NodeConfigProcessSpec{
			State: nodeconfig.StatusProvisioning,
		},
	}
)
...
var _ = Describe("ConfigManager", func() {
	...
	Context("WatchConfigs() should", func() {
		...
		It("return error to errCh if there was an error getting config for node from memory", func() {
			...
			cmi := mock_config_map.NewMockConfigMapInterface(ctrl)
			cm.configsMap = cmi
	...
	Context("DirtyStartup() should", func() {
		...
		It("return no error if there is nothing to restore", func() {
			cm, _, _ := prepareObjects(processProvisioning)
			defer close(cm.changes)
			defer close(cm.deletedNodes)
		...
		It("return error if cannot get slice from ConfigMap", func() {
			node := &corev1.Node{
				ObjectMeta: metav1.ObjectMeta{
					Name: nodeName,
				},
			...
			invalidConfig := v1alpha1.NewEmptyConfig(nodeName + nodeconfig.BackupSuffix)

			cm, _, _ := prepareObjects(processProvisioning, node, invalidConfig)
			defer close(cm.changes)
			defer close(cm.deletedNodes)
			...
			cmi.EXPECT().Store(gomock.Any(), gomock.Any())
			cmi.EXPECT().GetSlice().Return(nil, fmt.Errorf("error"))
		...
		It("return no error if restored configs successfully", func() {
			...
			cm, _, _ := prepareObjects(processProvisioning, node, invalidConfig, backupConfig, currentConfig)
			defer close(cm.changes)
			defer close(cm.deletedNodes)
			...
			cmi.EXPECT().Store(gomock.Any(), gomock.Any())
			cmi.EXPECT().GetSlice().Return([]nodeconfig.ConfigInterface{cfg}, nil)
			cfg.EXPECT().GetDeployed().Return(true)
			cfg.EXPECT().SetBackupAsNext().Return(true)
			cfg.EXPECT().SetDeployed(false)
			cfg.EXPECT().GetActive().Return(true)
			next := v1alpha1.NewEmptyConfig(nodeName)
			next.Spec.RoutingTable = []v1alpha1.RoutingTableSpec{{TableID: 1}}
