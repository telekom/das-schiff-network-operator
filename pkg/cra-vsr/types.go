...
//
//	system fast-path virtual-port fpvhost <name> [profile <p>]
//	    sockpath <path> sockmode <mode>
//
// SockPath is the host-side unix socket allocated by the 6WIND device plugin
// (never the pod-side path: that one only resolves inside the workload's mount
// namespace). SockMode is rendered from the VSR fast-path perspective (already
// inverted from the workload's view by the caller).
type FpvhostVirtualPort struct {
	Name       string  `xml:"name"`
	Profile    *string `xml:"profile,omitempty"`
	SocketPath *string `xml:"sockpath,omitempty"`
	SocketMode *string `xml:"sockmode,omitempty"`
}
