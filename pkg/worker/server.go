...
type Server struct {
	adapter Adapter
	workerpb.UnimplementedAgentServer
	logger *logr.Logger
}
...
//go:generate mockgen -destination ./mock/mock_worker.go . Client
type Client interface {
	SendConfig(context.Context, *v1alpha1.NodeNetworkConfig) error
}

func NewServer(adapter Adapter, logger *logr.Logger) *Server {
	sLog := logger.WithName("worker-server")
	return &Server{
		adapter: adapter,
		logger:  &sLog,
	}
...
// nolint: wrapcheck
func (s Server) SetConfiguration(_ context.Context, nc *workerpb.NetworkConfiguration) (*emptypb.Empty, error) {
	...
	var nodeCfg v1alpha1.NodeNetworkConfig
