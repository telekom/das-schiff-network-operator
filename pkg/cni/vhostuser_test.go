...
func TestWriteAndRemoveDeviceInfo(t *testing.T) {
	...
	att := &vhostUserAttachment{
		DeviceID: "3f9a2b1c7d",
		HostPath: "/run/vsr-vhost-user/3f9a2b1c7d/socket",
		PodPath:  "/run/vsr-virtio-user/0/socket",
		Mode:     SocketModeServer,
	}
	...
	// The POD-side path must be published: the host path does not resolve inside
	// the guest's mount namespace.
	if info.VhostUser == nil || info.VhostUser.Path != att.PodPath || info.VhostUser.Mode != att.Mode {
		t.Errorf("VhostUser = %+v, want path=%s mode=%s", info.VhostUser, att.PodPath, att.Mode)
	}
...
func TestWriteDeviceInfoNoFile(t *testing.T) {
	...
	conf := &NetConf{Transport: TransportVhostUser}
	att := &vhostUserAttachment{HostPath: "/run/x.sock", PodPath: "/run/y.sock", Mode: SocketModeClient}
	if err := writeDeviceInfo(conf, att); err != nil {
