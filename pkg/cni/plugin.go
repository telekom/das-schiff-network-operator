...
const About = "cni-workload: routed (no shared L2) or L2 access/trunk secondary attachment for KubeVirt VMs and pods"
...
func CmdAdd(args *skel.CmdArgs) (retErr error) {
	...
	if err := prepareIPAMResult(conf, result, gwV4, gwV6); err != nil {
		return err
	}
...
// rewriteRoutedResult makes the result describe the datapath a routed
// attachment actually gets. The IPAM plugin fills in its pool's gateway and
// whatever routes it was configured with, but neither is programmed here: the
// pod interface only carries on-link default routes via the CRA link-local
// gateways (installOnLinkDefaults), and the pool gateway does not exist on
// the isolated veth. Consumers of the result — the runtime's network-status
// annotation, KubeVirt configuring the guest — would otherwise be told a
// next-hop the workload cannot reach, so the gateways are replaced by the
// link-local ones and the routes by the defaults that are installed, per
// address family present.
func rewriteRoutedResult(result *current.Result, gwV4, gwV6 net.IP) {
	const v4Bits, v6Bits = 32, 128
	for i := range result.IPs {
		if result.IPs[i].Address.IP.To4() != nil {
			result.IPs[i].Gateway = gwV4
		} else {
			result.IPs[i].Gateway = gwV6
		}
	}
	...
	if haveV4 {
		result.Routes = append(result.Routes, &types.Route{
			Dst: net.IPNet{IP: net.IPv4zero, Mask: net.CIDRMask(0, v4Bits)},
			GW:  gwV4,
		})
	}
	if haveV6 {
		result.Routes = append(result.Routes, &types.Route{
			Dst: net.IPNet{IP: net.IPv6zero, Mask: net.CIDRMask(0, v6Bits)},
			GW:  gwV6,
		})
	}
}
...
// are narrowed to host prefixes — which is also what the agent exports — and
// the gateways and routes are rewritten to the on-link defaults that are
// actually installed. An L2 attachment lives in the pool's subnet and keeps
// it, gateway and routes included; its IPAM routes are applied to the pod
// interface, which needs an address to reach them from.
func prepareIPAMResult(conf *NetConf, result *current.Result, gwV4, gwV6 net.IP) error {
	if conf.isL2() {
