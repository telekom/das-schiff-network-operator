...
func loadNodeConfig(path string) (*NodeConfig, error) {
	...
	// Exactly one document: anything after the object (a second object,
	// stray tokens, malformed text) would be silently ignored by a single
	// Decode, so the decoder has to be at EOF once the object is read.
	if err := dec.Decode(new(json.RawMessage)); !errors.Is(err, io.EOF) {
		return nil, fmt.Errorf("failed to parse node config %s: trailing data after the JSON object", path)
	}
	return cfg, nil
}
