...
const (
	...
	// defaultDeviceInfoDPDir is where a device plugin writes its device-info
	// files, by the k8snetworkplumbingwg convention.
	defaultDeviceInfoDPDir = "/var/run/k8s.cni.cncf.io/devinfo/dp"
...
// deviceInfoDPDir is the device plugin's device-info directory; a variable so
// tests can point it at a scratch directory.
var deviceInfoDPDir = defaultDeviceInfoDPDir
...
// authoritative first. The device plugin's own file is keyed by the deviceID
// in its name, so it describes exactly the socket this attachment was handed
// (the file body carries no device identity). The copy Multus stages under
// CNIDeviceInfoFile is consulted second: it is best-effort (its failures are
// only logged) and, for a pod with several attachments to one resource, its
// pairing with the deviceID rests on Multus walking network-status and the
// allocation in the same order, which is not something this plugin can verify.
func stagedDeviceInfoPaths(conf *NetConf, deviceID string) []string {
	...
	paths := []string{filepath.Join(deviceInfoDPDir, name)}
