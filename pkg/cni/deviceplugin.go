...
type vhostUserAttachment struct {
	...
	// device plugin's own device-info file is authoritative for it.
	Mode string
}
...
// directory) supplies the pod-side path and the socket mode. Only when the
// device-info files are unreadable is the pod-side path derived positionally,
// which cannot be right for a pod holding several sockets.
//
...
func resolveVhostUser(conf *NetConf) (*vhostUserAttachment, error) {
	...
	// The device plugin's own statement of what it allocated wins over both the
	// derived index and the NAD's socketMode claim.
