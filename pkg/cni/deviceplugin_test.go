...
func TestResolveVhostUserPrefersRuntimeConfigDeviceID(t *testing.T) {
	conf := &NetConf{Transport: TransportVhostUser, DeviceID: "toplevel00", SocketMode: SocketModeServer}
...
func TestResolveVhostUserDerivesPathsFromDeviceID(t *testing.T) {
	...
	conf := &NetConf{Transport: TransportVhostUser, DeviceID: "3f9a2b1c7d", SocketMode: SocketModeClient}
	att, err := resolveVhostUser(conf)
	if err != nil {
		t.Fatalf("resolveVhostUser: %v", err)
	}
	...
	if att.Mode != SocketModeClient {
		t.Errorf("Mode = %q, want the NAD-selected %q", att.Mode, SocketModeClient)
	}
}
...
func TestResolveVhostUserUsesNADSocketMode(t *testing.T) {
	staged := filepath.Join(t.TempDir(), "cni-device-info.json")
	writeStagedDeviceInfo(t, staged, SocketModeServer, "/run/vsr-virtio-user/2/socket")
	...
	// Device info is authoritative for the pod-side index, but the current
	// plugin always reports server for its mode. The NAD selects socket_mode.
...
func TestResolveVhostUserIgnoresUnusableDeviceInfo(t *testing.T) {
	...
	for name, path := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			conf := &NetConf{Transport: TransportVhostUser, DeviceID: "3f9a2b1c7d", SocketMode: SocketModeServer}
...
// TestResolveVhostUserPrefersDevicePluginFile covers the precedence between
// the two device-info sources: the plugin's own file is named after the
// deviceID and therefore describes this very socket, while Multus's staged copy
// is only paired with the attachment positionally.
func TestResolveVhostUserPrefersDevicePluginFile(t *testing.T) {
	dpDir := t.TempDir()
	orig := deviceInfoDPDir
	deviceInfoDPDir = dpDir
	t.Cleanup(func() { deviceInfoDPDir = orig })

	writeStagedDeviceInfo(t, filepath.Join(dpDir, "nc-k8s-plugin.6wind.com-virtio-user-3f9a2b1c7d-device.json"),
		SocketModeServer, "/run/vsr-virtio-user/1/socket")
	staged := filepath.Join(t.TempDir(), "cni-device-info.json")
	writeStagedDeviceInfo(t, staged, SocketModeServer, "/run/vsr-virtio-user/0/socket")
	...
	if want := "/run/vsr-virtio-user/1/socket"; att.PodPath != want {
		t.Errorf("PodPath = %q, want the device plugin's %q", att.PodPath, want)
	}
}
