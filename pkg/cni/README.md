...
`l2.<vlanID>` link-interface). No VNI or VLAN id is needed in the CNI config; the
...
4. Multus best-effort copies the plugin's device-info file
   ...
   our `CNIDeviceInfoFile`. For the pod-side socket path we read the plugin's
   own `dp/` file first — its name carries the `deviceID`, so it describes
   exactly the socket we were handed — and fall back to the staged copy, whose
   pairing with the attachment is only positional for a pod with several
   attachments to one resource. Current 6WIND device-plugin versions always
   report `server` as the device-info mode, so the required NAD userdata
   `socket_mode` is the **only** source of the socket role: it states the
   workload's side, the VSR side is its inverse, and the `device-info` file the
   plugin publishes for the pod echoes the workload's side. A missing or
   malformed file is tolerated and the derived path is used.
5. The CNI result (and thus the `k8s.v1.cni.cncf.io/network-status` annotation)
   carries a **deterministic, locally administered MAC** derived from the
   container id and interface name. There is no kernel interface to read one
   from, but KubeVirt and DPDK launchers (e.g. 6WIND's `run_dpdk.py`) pass the
   annotation's `mac` to the virtio-user device, so the address a workload runs
   with — and the MAC EVPN learns for an L2 attachment — is stable.
...
- **cra-vsr:** the VSR fast path owns the FIB, so the moved port cannot be
  ...
  virtual-port fpvhost fpvhost-<deviceID> socket-mode <inverted>`, where
...
`config/default`. A deployment that wants workload attachments must therefore:
