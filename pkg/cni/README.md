# cni-workload — routed or L2 CNI for KubeVirt VMs (and pods)

`cni-workload` gives a workload (a KubeVirt VM, or a pod) a secondary interface
whose CRA-side veth end is moved into the CRA network namespace. In the default
**routed** mode there is **no shared L2**: the workload gets a real routable
IPv4 `/32` + IPv6 `/128` and the routing daemon (FRR / 6WIND VSR) advertises
on-link host routes to it via BGP. In **L2 attach mode** the port instead joins
one or more existing Layer2 bridges as an access or 802.1Q trunk port (see
[L2 attach mode](#l2-attach-mode)).
...
install a connected route on the isolated veth instead. For the same reason the
result's per-address `gateway` and its `routes` are rewritten to what is
actually programmed — the link-local gateway of each family and one on-link
default route (`0.0.0.0/0`, `::/0`) per family present — instead of the IPAM
pool's gateway and routes, which the isolated veth cannot reach. Anything
reading the result (the network-status annotation, KubeVirt configuring the
guest) therefore sees the same next-hop the pod interface uses.
...
| `mtu`               | no       | `1500`  | veth MTU, `1000`–`9216` (at least `1280` when the IPAM result carries an IPv6 address); in L2 mode every attached domain must be able to carry it (see [L2 trunking](#l2-trunking)) |
...
| ---------------- | ------- | ----------- |
| `agentSocket`    | `/run/das-schiff/workload-cni.sock` | unix socket of the node-local CRA agent; must match the agent's `WORKLOAD_CNI_SOCKET` env (same default) and lie in the `/run/das-schiff` hostPath the agent mounts |
