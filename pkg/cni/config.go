...
// Package cni implements the "cni-workload" CNI plugin. It provides a
// secondary attachment whose CRA-side veth end is moved into the CRA network
// namespace. In the default routed mode there is no shared L2: the workload (a
// KubeVirt VM via the built-in bridge binding, or a pod) gets a real routable
// IPv4 /32 + IPv6 /128 and the routing daemon (FRR / VSR) advertises on-link
// host routes to it via BGP. In L2 attach mode the port joins one or more
// existing Layer2 bridges as an access or 802.1Q trunk port instead.
package cni
...
const (
	...
	// NodeWorkloadPorts schema (MaxItems on layer2Trunk), so an oversized
	// config is rejected before any interface or IPAM state exists.
	maxLayer2TrunkMembers = 64
...
type NetConf struct {
	...
	// when AttachMode is "l2", and neither may be set in any other mode.
...
// only accepted from the node config. Keys are matched case-insensitively:
// encoding/json decodes NetConf with case-insensitive field matching, so
// "TrunkInterface" would otherwise land in the struct unchecked.
		...
		for _, key := range nodeOnlyKeys {
			if strings.EqualFold(present, key) {
				return fmt.Errorf("%q is a node-level setting read from %s and is not accepted from the "+
					"network attachment configuration", present, NodeConfigPath)
			}
		}
	}
	return nil
...
// validateTrunk rejects oversized trunks, duplicate members and out-of-range
// or colliding workload-side VLAN ids. Members that inherit their id (vlan unset) can only
...
func validateTrunk(members []Layer2TrunkMember) error {
	if len(members) > maxLayer2TrunkMembers {
		return fmt.Errorf("layer2Trunk has %d members (max %d)", len(members), maxLayer2TrunkMembers)
	}
