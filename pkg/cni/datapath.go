...
// addRoute installs a route without replacing an existing one. The workload
// interface is a secondary attachment, so a route the pod already has (most
// notably the primary interface's default route) belongs to another CNI; a
// prefix/table/metric collision therefore fails the ADD instead of hijacking it.
