...
func TestParseConfigErrors(t *testing.T) {
	tests := map[string]string{
		...
		// encoding/json matches struct fields case-insensitively, so any
		// spelling that would populate NetConf has to be refused.
		"TrunkInterface mixed-case in NAD": `{"cniVersion":"1.0.0","type":"cni-workload","ipam":{"type":"host-local"},"TrunkInterface":"eth0"}`,
		"AGENTSOCKET upper-case in NAD":    `{"cniVersion":"1.0.0","type":"cni-workload","ipam":{"type":"host-local"},"AGENTSOCKET":"/tmp/evil.sock"}`,
		"CraNetns mixed-case in NAD":       `{"cniVersion":"1.0.0","type":"cni-workload","ipam":{"type":"host-local"},"CraNetns":"/proc/1/ns/net"}`,
	}
...
func TestLoadNodeConfig(t *testing.T) {
	...
	for _, body := range []string{`{} {"agentSocket":"/tmp/x"}`, `{} }`, `{} garbage`} {
		if _, err := loadNodeConfig(write("trailing.json", body, 0o644)); err == nil {
			t.Errorf("trailing data after the JSON object must be rejected: %q", body)
		}
	}
...
func TestParseConfigModeErrors(t *testing.T) {
	tests := map[string]string{
		...
		"trunk too many":       oversizedTrunkConf(),
...
// oversizedTrunkConf builds an L2 trunk config with one member more than the
// schema allows, each member distinct and with its own VLAN.
func oversizedTrunkConf() string {
	members := make([]string, 0, maxLayer2TrunkMembers+1)
	for i := range maxLayer2TrunkMembers + 1 {
		members = append(members, fmt.Sprintf(`{"name":"l2-%d","vlan":%d}`, i, i+1))
	}
	return `{"cniVersion":"1.0.0","type":"cni-workload","attachMode":"l2","layer2Trunk":[` + strings.Join(members, ",") + `]}`
}
