...
func cmdAddVhostUser(conf *NetConf, args *skel.CmdArgs) error {
	...
	// Consumers of the network-status annotation (KubeVirt, DPDK launchers such
	// as 6WIND's run_dpdk.py) read the interface MAC from the result and hand it
	// to the virtio-user device, so the result must carry one even though no
	// kernel interface exists.
	result.Interfaces = []*current.Interface{{
		Name:    args.IfName,
		Mac:     vhostMAC(args.ContainerID, args.IfName).String(),
		Sandbox: args.Netns,
	}}
