...
//      which persists it to NodeWorkloadPorts and triggers the grout reconcile.
...
// comfortably exceed a reconcile round-trip (NodeWorkloadPorts write → watch →
...
func cmdAddGroutTap(conf *NetConf, args *skel.CmdArgs) error {
	...
	// optional in the L2 attach mode, where the workload is addressed inside the
	// shared L2 domain rather than by this plugin.
	...
	// Deliberately no IPv4 gateway. grout has one node-global IPv4 address
	// table with no per-interface scope for link-local space, so the shared
	// 169.254.1.1/32 can only ever exist on one port: the second routed
	// attachment on a node would be rejected with EADDRINUSE, and because a
	// failed grcli line aborts the rest of the batch and invalidates the
	// NodeNetworkConfig revision, that failure stops config deployment for the
	// whole cluster. The pod instead reaches the fabric over IPv4 with the
	// per-interface-scoped fe80::1 as its next-hop (see v4NextHopIsV6), which
	// needs no IPv4 address on the CRA side at all.
	var gwV4 net.IP

	// Ask the agent to create the grout net_tap in the CRA netns. It returns the
	// kernel netdev name of that tap, which is deliberately distinct from the
	// grout port name (grout names its own control plane representor after the
	// port). Never guess it: waiting on the port name would either time out or
	// hijack the representor, so an agent that does not report one is an error.
	port := portName(args.ContainerID, args.IfName)
	tapName, err := notifyAgentAdd(conf, args, port, gwV4, gwV6, result, nil)
	if err != nil {
		return err
	}
	if tapName == "" {
		return fmt.Errorf("agent did not report a tap name for port %q", port)
	}
	// From here on, drop the agent-side attachment on any failure.
	defer func() {
		if !success {
			_ = notifyAgentDel(conf, args, port)
		}
	}()
...
//
// Like the veth DEL path every step runs and the failures are aggregated: an
// early return would leak the NodeWorkloadPorts entry (and with it the grout
// net_tap) whenever IPAM release fails.
func cmdDelGroutTap(conf *NetConf, args *skel.CmdArgs) error {
	...
	if err := notifyAgentDel(conf, args, portName(args.ContainerID, args.IfName)); err != nil {
...
func adoptGroutTap(conf *NetConf, args *skel.CmdArgs, craNetnsPath, tapName string, result *current.Result) (*current.Interface, error) {
	...
	err = ns.WithNetNSPath(args.Netns, func(_ ns.NetNS) error {
		...
		// Size the tap like the veth path sizes its pod-side end. The CRA sizes
		// the port (and any VLAN sub-interface it derives from it for a trunk
		// member) from the same value, so both ends of the tap agree.
		if merr := netlink.LinkSetMTU(link, conf.mtu()); merr != nil {
			return fmt.Errorf("failed to set grout tap MTU to %d: %w", conf.mtu(), merr)
		}
