...
func NewManager(craURLs []string, timeout time.Duration, clientCert, clientKey string) (*Manager, error) {
	...
	// A malformed PEM leaves the pool empty, which fails closed but with a
	// bare "unknown authority" at the first request rather than at the
	// misconfiguration -- so reject it here, where the cause is still visible.
	if !caCertPool.AppendCertsFromPEM(clientCertData) {
		return nil, fmt.Errorf("no certificates found in client cert file %q", clientCert)
	}
