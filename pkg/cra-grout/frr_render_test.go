...
// TestGroutFRRTemplateMatchesCRAFRR guards against the grout FRR template
// drifting from the cra-frr one. grout only replaces the fast path; its control
// plane is the same FRR rendered from the same template, so the two overlays
// must ship byte-identical files. kustomize refuses both a symlink and a
// relative file source outside the overlay directory, so the file is a copy --
// and a copy silently fell behind cra-frr once already (it was missing the
// underlay workload-port export and the management-VRF peers).
func TestGroutFRRTemplateMatchesCRAFRR(t *testing.T) {
	grout, err := os.ReadFile("../../config/agent-cra-grout/frr.conf.tpl")
	if err != nil {
		t.Fatalf("reading grout template: %v", err)
	}
	frr, err := os.ReadFile("../../config/agent-cra-frr/frr.conf.tpl")
	if err != nil {
		t.Fatalf("reading cra-frr template: %v", err)
	}
	if !bytes.Equal(grout, frr) {
		t.Fatal("config/agent-cra-grout/frr.conf.tpl drifted from config/agent-cra-frr/frr.conf.tpl; " +
			"re-copy it (grout shares the cra-frr control plane verbatim)")
	}
}
