...
// The exported set must not depend on port order or on a route being carried
// by more than one port (e.g. during a live migration handover): otherwise the
// rendered config differs between reconciles and FRR reloads for nothing.
func TestTemplateFRRWorkloadExportIsStableAndDeduplicated(t *testing.T) {
	render := func(ports ...v1alpha1.WorkloadPort) string {
		return renderFRR(t, true, &v1alpha1.NodeNetworkConfigSpec{GlobalWorkloadPorts: ports})
	}
	a := v1alpha1.WorkloadPort{Interface: "cra00000a", HostRoutes: []string{"10.201.0.20/32", "10.201.0.10/32"}}
	b := v1alpha1.WorkloadPort{Interface: "cra00000b", HostRoutes: []string{"10.201.0.10/32", "fd00:201::10/128"}}

	rendered := render(a, b)
	if rendered != render(b, a) {
		t.Errorf("rendered config depends on workload port order")
	}
	const shared = "ip prefix-list pl_workload_export permit 10.201.0.10/32"
	if got := strings.Count(rendered, shared); got != 1 {
		t.Errorf("expected exactly one %q, got %d", shared, got)
	}
	if first, second := strings.Index(rendered, "permit 10.201.0.10/32"), strings.Index(rendered, "permit 10.201.0.20/32"); first < 0 || second < first {
		t.Errorf("expected host routes to be emitted in sorted order, got:\n%s", rendered)
	}
}
