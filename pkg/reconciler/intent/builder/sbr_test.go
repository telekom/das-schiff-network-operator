...
func TestComputeAggregatePrefix(t *testing.T) {
	int32Ptr := func(v int32) *int32 { return &v }

	// No override — return CIDR as-is.
	assert.Equal(t, "10.250.0.1/24", computeAggregatePrefix("10.250.0.1/24", nil))

	// Override more specific than CIDR — apply it.
	assert.Equal(t, "10.250.0.0/28", computeAggregatePrefix("10.250.0.1/24", int32Ptr(28)))

	// Override less specific (broader) than CIDR — clamp to CIDR.
	assert.Equal(t, "10.250.0.1/24", computeAggregatePrefix("10.250.0.1/24", int32Ptr(20)))

	// Override equal to CIDR — no change.
	assert.Equal(t, "10.250.0.1/24", computeAggregatePrefix("10.250.0.1/24", int32Ptr(24)))

	// IPv6: no override.
	assert.Equal(t, "fd94:685b:30cf:501::1/64", computeAggregatePrefix("fd94:685b:30cf:501::1/64", nil))

	// IPv6: more specific override.
	assert.Equal(t, "fd94:685b:30cf:501::/80", computeAggregatePrefix("fd94:685b:30cf:501::1/64", int32Ptr(80)))

	// IPv6: broader override clamped.
	assert.Equal(t, "fd94:685b:30cf:501::1/64", computeAggregatePrefix("fd94:685b:30cf:501::1/64", int32Ptr(48)))
}
