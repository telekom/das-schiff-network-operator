...
// (the node's default/underlay table, fabric VRFs and local VRFs) into the
// netlink workload-port list the frr-cra server programs.
//
// It mirrors exactly the placements workloadcni.MergeIntoNodeNetworkConfig
// makes. The cluster VRF is platform-owned and never a placement target, so
// ClusterVRF.WorkloadPorts is deliberately not read: the merge does not clear
// it either, and realising it here would let a stale or hand-written entry
// reach the datapath without any owner to remove it again.
func (*CRAFRRConfigApplier) convertWorkloadPorts(nodeCfg *v1alpha1.NodeNetworkConfig) []nl.WorkloadPort {
...
func NewNodeNetworkConfigReconciler(
	...
	commonReconciler, err := common.NewNodeNetworkConfigReconciler(
		...
		common.ReconcilerOptions{
			...
			ReservedVRFs:        []string{baseConfig.ManagementVRF.Name, baseConfig.ClusterVRF.Name},
		},
	)
	if err != nil {
