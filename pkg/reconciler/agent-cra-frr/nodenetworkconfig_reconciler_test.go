...
// the primary use case - and from nowhere else: the platform-owned cluster VRF
// is not a placement target, so ports found there must not reach the datapath.
func TestConvertWorkloadPortsCoversEveryTable(t *testing.T) {
	...
	nodeCfg := &v1alpha1.NodeNetworkConfig{
		Spec: v1alpha1.NodeNetworkConfigSpec{
			...
			// Never placed by the merge; must be ignored.
