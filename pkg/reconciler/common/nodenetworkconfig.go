...
type ReconcilerOptions struct {
	...
	// The CRA agent that owns the CRA-side L3 programming sets it (today
	// cra-frr, via netlink; cra-vsr NETCONF rendering is a follow-up). Leave
	// nil to disable.
	WorkloadPortsSource WorkloadPortsSource

	// ReservedVRFs names the platform-owned VRFs (management and cluster VRF of
	// the CRA base config). Workload ports recorded against one of them are
	// dropped before the merge (along with any otherwise invalid entry) instead
	// of being realised as a local VRF that would adopt the platform's device of
	// that name.
	ReservedVRFs []string
}
...
func NewNodeNetworkConfigReconciler(
	...
	reconciler := &NodeNetworkConfigReconciler{
		...
		reservedVRFs:              opts.ReservedVRFs,
	}
...
// ReservedVRFs returns the platform-owned VRF names workload ports may not
// target, for the workload-cni server to refuse them at ADD.
func (r *NodeNetworkConfigReconciler) ReservedVRFs() []string {
	return r.reservedVRFs
}
