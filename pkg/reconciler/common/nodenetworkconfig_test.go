...
var _ = Describe("NodeNetworkConfigReconciler", func() {
	...
	Context("SetStatus", func() {
		...
		It("should preserve in-memory Spec changes not persisted to the server", func() {
			// Regression guard: merged workload ports live only in the in-memory
			...
			Expect(cfg.Spec.ClusterVRF.WorkloadPorts).To(HaveLen(1))
			Expect(cfg.Spec.ClusterVRF.WorkloadPorts[0].Interface).To(Equal("cra0123456789ab"))
		})
	})
