...
func (r *reconcileNodeNetworkConfig) configureFRR(vrfConfigs []frr.VRFConfiguration) error {
...
func (r *reconcileNodeNetworkConfig) createVrfConfigMap(l3vnis []networkv1alpha1.VRFRouteConfigurationSpec) (map[string]frr.VRFConfiguration, error) {
	...
	for i := range l3vnis {
		...
		if vni == 0 && vni > 16777215 {
			...
			r.Logger.Error(err, "VNI can not be set to 0, ignoring", "vrf", spec.VRF)
			continue
		}
...
func (r *reconcileNodeNetworkConfig) gatherInterfacesInfo(vrfConfigs []frr.VRFConfiguration, existing []nl.VRFInformation) (preexisting, toDelete []nl.VRFInformation) {
	// Check for VRFs that are configured on the host but no longer in Kubernetes
