...
type ConfigInterface interface {
	...
	DeleteInvalid(ctx context.Context, c client.Client) error
...
func (nc *Config) deployNext(ctx context.Context, c client.Client, logger logr.Logger) (bool, error) {
	...
	skip, err := nc.createOrUpdate(ctx, c, logger)
	if err != nil {
...
func (nc *Config) DeleteInvalid(ctx context.Context, c client.Client) error {
	nc.mtx.Lock()
	defer nc.mtx.Unlock()
	invalidName := fmt.Sprintf("%s%s", nc.name, InvalidSuffix)
	...
	if err := c.Get(ctx, types.NamespacedName{Name: invalidName}, &config); err != nil {
		if apierrors.IsNotFound(err) {
			// invalid config for the node does not exist
			if nc.invalid != nil {
				nc.invalid = nil
			}
			return nil
		}
		return fmt.Errorf("error getting invalid config: %w", err)
	}
