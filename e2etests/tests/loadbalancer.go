	...
	Context("m2m VRF (TC-06)", func() {
		...
		It("should be reachable via LoadBalancer VIP from m2mgw", func() {
			...
			Eventually(func() error {
				...
				if statusCode != "200" {
					return fmt.Errorf("expected 200, got %s", statusCode)
				}
				return nil
			}).WithTimeout(60*time.Second).WithPolling(5*time.Second).Should(Succeed(),
				"Expected HTTP 200 from LB VIP IPv4")
			...
			Eventually(func() error {
				...
				"Expected HTTP 200 from LB VIP IPv6")
	...
	Context("c2m VRF (TC-06b)", func() {
		...
		It("should be reachable via LoadBalancer VIP from c2mgw", func() {
			...
			Eventually(func() error {
				...
				"Expected HTTP 200 from c2m LB VIP IPv4")
			...
			Eventually(func() error {
				...
				"Expected HTTP 200 from c2m LB VIP IPv6")
