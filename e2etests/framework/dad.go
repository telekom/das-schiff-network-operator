...
func (f *Framework) awaitIPv6DADCompletion(ctx context.Context, namespace, podName, ipv6Addr, ifName string, timeout time.Duration, exec execInPodFunc) error {
	...
	return Poll(ctx, 2*time.Second, func() (bool, error) {
		stdout, stderr, err := exec(ctx, namespace, podName, "", []string{
...
func (*Framework) addIPv6AddressWithExec(ctx context.Context, namespace, podName, cidr, ifName string, options []string, exec execInPodFunc) error {
