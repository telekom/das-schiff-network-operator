...
COPY cmd/cni-workload/main.go main.go
COPY api/ api/
COPY pkg/ pkg/

# Build a static binary (the plugin runs on the host, not in this image).
RUN CGO_ENABLED=0 GOOS=linux go build -a -o cni-workload main.go
...
WORKDIR /
COPY --from=builder /workspace/cni-workload /usr/local/bin/cni-workload
