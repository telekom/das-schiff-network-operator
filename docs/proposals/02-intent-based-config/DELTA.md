...
| **Pipeline integration as open question** (§3.4) | Option A (generate low-level CRDs) vs. Option B (extend `ConfigReconciler` directly). **RESOLVED: Option B** (D24). | The input assumed the controller would produce the "already existing configuration resources" (Option A). Option B is chosen instead — the `ConfigReconciler` is extended to watch intent CRDs directly. No intermediate low-level CRDs are generated. Low-level CRDs remain as a user-managed escape hatch. |
...
| Input | Proposal |
|---|---|
| Assumed: intent controller produces "already existing configuration resources" (i.e., `Layer2NetworkConfiguration`, `VRFRouteConfiguration`, `BGPPeering`) — equivalent to **Option A** | **Resolved: Option B** (D24). The `ConfigReconciler` is extended to watch intent CRDs directly — no intermediate low-level CRDs are generated. Low-level CRDs remain as a user-managed escape hatch. Conflicting low-level CRDs are rejected when intent CRDs cover the same scope (D32). |
