...
### 3.4 Integration with the Revision Pipeline — Resolved (Option B)
...
#### Decision: Option B (D24)

**Option B is chosen.** The `ConfigReconciler` is extended to **also watch** intent CRDs. At revision time, it reads both the intent CRDs and the existing low-level CRDs (escape hatch), merging them into a single `NetworkConfigRevision`. No intermediate low-level resources are generated by intent controllers.

Low-level CRDs (`Layer2NetworkConfiguration`, `VRFRouteConfiguration`, `BGPPeering`) remain available as a direct-use escape hatch for advanced users. They are **not generated** by the intent controllers — they exist only when users create them manually.
...
With **Option B** (D24), the `ConfigReconciler` resolves `Collector` + `TrafficMirror` directly at `NodeNetworkConfig` build time, populating GRE interfaces, loopbacks, EVPN export filters, and `MirrorACLs` on target nodes. No intermediate `MirrorTarget` / `MirrorSelector` resources are generated.

The low-level `MirrorSelector` / `MirrorTarget` CRDs remain available as an escape hatch for advanced use cases (e.g., custom GRE naming, multiple loopbacks per VRF, complex multi-target scenarios).
...
4. **Set up routes** toward each matched destination's VRF (resolved directly into revision data by `ConfigReconciler`, D24).
...
4. **Feed into `ConfigReconciler`** (Option B, D24):
   - Mirror VRF: the `ConfigReconciler` reads the `Collector` + its referenced `VRF` and injects loopback with allocated IP, GRE interface (src = loopback, dst = collector address), EVPN export filter entry into the revision.
...
4. **Feed into `ConfigReconciler`** (Option B, D24) — the `ConfigReconciler` reads `TrafficMirror` and generates `MirrorACL` entries on the source L2 or VRF in the revision, with mirrorDestination = collector's GRE interface name.
...
1. ~~**Pipeline integration approach (§3.4):**~~ **RESOLVED — Option B (do not feed into low-level CRDs; keep them as an option).** The `ConfigReconciler` is extended to watch intent CRDs directly and merge them into the revision alongside any user-created low-level CRDs. No intermediate low-level CRDs are generated. Low-level CRDs remain available as a direct-use escape hatch for advanced users. See D24.
...
| D24 | **Pipeline integration: Option B — `ConfigReconciler` watches intent CRDs directly** — no intermediate low-level CRDs generated; low-level CRDs remain as a user-managed escape hatch | Architecturally cleaner: single layer, no generated artifacts, no ownership/GC complexity. Simpler debugging path (intent CRD → revision → node config). Low-level CRDs are not generated but remain available for advanced users who create them manually. Resolves OQ 1 |
