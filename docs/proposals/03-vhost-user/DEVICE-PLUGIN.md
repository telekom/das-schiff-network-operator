...
- **§8** — historical implementation hazards and the current socket-mode
  limitation.
...
> input**, and everything else — the host socket path and the pod socket path —
> is either derived from it or read from the device-info file that Multus has
> already staged for us. The current socket mode is selected separately from
> NAD userdata `socket_mode` (§7.1, §8.3).
...
- **A device-info JSON file** written by the plugin under
  ...
  **[established]**. The filename of the JSON itself is fixed by the NPWG client
  ...
  Current 6WIND plugin versions always write `mode: "server"` regardless of the
  required socket role. Treat this value as **[unverified]** and non-authoritative
  — see §8.3.
...
2. **Try the staged device-info file first.** Multus has likely already copied
   ...
   take the socket path from it — it is the plugin's own statement of which
   pod-side socket path it allocated.

3. **Take the socket role from NAD userdata `socket_mode`.** Current 6WIND
   device-plugin versions always report `server` in device-info, so that value
   cannot choose the VSR polarity. The CNI validates `socket_mode` as `client`
   or `server` and publishes that selected workload-side mode. A future
   device-plugin version may become a source only after it reports the actual
   per-allocation role and is verified on hardware.

4. **Otherwise derive the paths from the `deviceID`.** The two paths are *not*
...
5. **Publish the device-info file for downstream.** Write the **pod-side** path
   with the mode selected from NAD userdata.
   Writing the host path here yields a domain XML pointing at a socket the VM
   cannot open.

6. **Hand the host-side path to the agent** over the existing routedcni gRPC
...
## 8. Historical implementation hazards and current limitations

### 8.1 `socketPath` must not be a required static NAD field

The original design required `socketPath` and the reference NAD set:
...
that silently never connects. The current implementation derives the host path
from `deviceID`; `socketPath` remains an optional **override** for
non-device-plugin setups.
...
The current implementation reads that file for the pod-side path before falling
back to derivation. The copy is best-effort and Multus ignores failures, so the
fallback remains necessary.

### 8.3 Current device-info mode is not authoritative

The current 6WIND device plugin always reports `mode: "server"` in device-info,
irrespective of the role the workload must use. Consuming that field would
render the fpvhost port with the wrong polarity whenever the workload is the
client. The CNI therefore requires NAD userdata `socket_mode` (`client` or
`server`) and uses it as the workload-side role; VSR receives the inverse.

The device-info mode remains available as a future source only if a
device-plugin variant is verified to report the actual per-allocation role.
...
1. **`mode` semantics.** Confirm the NAD-selected `socket_mode` polarity for
   `vhost-user` versus `virtio-user`, and whether the device plugin still always
   reports `server` in device-info. (§8.3)
...
- **Require NAD `socket_mode`.** Current device-info mode is always `server` and
  cannot select the workload-side role.
