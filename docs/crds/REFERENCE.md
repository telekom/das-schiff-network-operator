...
| `references` | []string | Names of Destinations that reference this VRF. |
...
| `nodeIPs.reservedRanges` | []string | | | IP ranges (CIDRs) reserved for pods, excluded from node allocation. |
...
| `anycast.gateways` | []string | Anycast gateway addresses (IPv4 and/or IPv6). |
...
| `rawNetplan` | \*RawExtension | | Escape hatch for netplan features not covered by typed fields. Merged into generated config. |
