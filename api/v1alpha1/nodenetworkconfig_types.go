...
type NodeNetworkConfigSpec struct {
	...
	// than a tenant/cluster L3VRF. Their host routes are installed as kernel
	// routes on the CRA-side port and exported into the underlay BGP through a
	// filtered kernel redistribution that permits exactly these prefixes.
...
type Layer2 struct {
	...
	// the CRA netns and added as bridge slaves) with no L3 addressing. Only the
	// cra-frr flavor renders them today; the cra-vsr rendering is a follow-up.
	// +optional
...
type VRF struct {
	...
	// CRA network namespace) bound into this VRF. The cra-frr agent programs them
	// via netlink; the cra-vsr flavor is expected to render them as infrastructure
	// interfaces plus interface-static routes via NETCONF (the fast path owns the
	// FIB there), which is not implemented yet.
...
type WorkloadPort struct {
	...
	// requires its Layer2 to carry at least this much, a trunk requires every
	// one of its members to (the ADD is rejected otherwise). Routed
	// attachments are not constrained by it.
	// +optional
	// +kubebuilder:validation:Minimum=0
	// +kubebuilder:validation:Maximum=9216
	MTU uint16 `json:"mtu,omitempty"`
	// GatewayV4 is the on-link IPv4 link-local gateway address (with prefix
	// length, e.g. "169.254.1.1/32") configured on the infrastructure interface.
	// +kubebuilder:validation:Format=cidr
	GatewayV4 string `json:"gatewayV4,omitempty"`
	// GatewayV6 is the on-link IPv6 link-local gateway address (with prefix
	// length, e.g. "fe80::1/128") configured on the infrastructure interface.
	// +kubebuilder:validation:Format=cidr
