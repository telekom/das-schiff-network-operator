...
// +kubebuilder:validation:XValidation:rule="(has(self.transport) && self.transport == 'vhostuser') ? (has(self.socketPath) && size(self.socketPath) > 0) : (!has(self.socketPath) && !has(self.socketMode))",message="socketPath is required for transport vhostuser; socketPath and socketMode are only valid for transport vhostuser"
