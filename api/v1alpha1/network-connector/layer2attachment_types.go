...
type SRIOVConfig struct {
	// Enabled controls whether SR-IOV is active. Immutable once set.
	Enabled bool `json:"enabled"`
}

// NodeIPConfig defines node IP assignment configuration for a Layer2Attachment.
type NodeIPConfig struct {
	// Enabled controls whether node IPs are assigned from the referenced Network.
	Enabled bool `json:"enabled"`

	// ReservedRanges lists IP ranges (CIDRs) within the Network that are reserved
	// for pod use and must not be allocated to nodes. Each entry must be a valid
	// CIDR that falls within the parent Network's address space.
	// +optional
...
type AnycastStatus struct {
	...
	// Gateways lists the anycast gateway addresses (IPv4 and/or IPv6).
...
type Layer2AttachmentSpec struct {
	...
	// SRIOV is the SR-IOV configuration. When set, the CRA agent skips
	// VXLAN/VLAN bridge setup and configures VF passthrough instead.
	// +optional
