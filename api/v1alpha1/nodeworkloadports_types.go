...
// rendering (netlink on cra-frr; cra-vsr NETCONF rendering is a follow-up).
// Storing the state in this aggregate per-node object makes it durable across
// agent restarts and directly observable, without a round-trip through the
// central intent pipeline.
type NodeWorkloadPortsSpec struct {
	// Ports is the list of workload attachments (routed or L2) currently live on the node.
...
type WorkloadPortEntry struct {
	...
	// means the underlay/default table. Must be empty in L2 attach mode (see
	// Layer2AttachmentRef): an L2 port carries no L3 state. The name is
	// rendered into the CRA configuration, so it is limited to a kernel
	// interface name.
