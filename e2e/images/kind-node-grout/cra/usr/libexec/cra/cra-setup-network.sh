...
# The underlay is ECMP: a session sourced from the node VTEP leaves over one
# uplink and its answer arrives on the other, which reverse path filtering drops
# even in loose mode. Turn it off for the namespace -- `default` so grout's
# control plane representors inherit it as they are created (the effective mode
# is max(all, <iface>), so this only works if nothing sets it per interface; see
# GROUT_OVERRIDE_RP_FILTER in das-schiff-cra-grout.Dockerfile).
ip netns exec cra sysctl -qw net.ipv4.conf.all.rp_filter=0 net.ipv4.conf.default.rp_filter=0 || true
