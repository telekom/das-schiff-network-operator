...
ip addr add 192.0.2.2/32 dev lo 2>/dev/null || true

# VRF mgmt
ip link add vr.mgmt type vrf table 100 2>/dev/null || true
ip link add vx.mgmt type vxlan id 20 dev lo local 192.0.2.2 dstport 4789 nolearning 2>/dev/null || true
...
ip link add vx.m2m type vxlan id 2002026 dev lo local 192.0.2.2 dstport 4789 nolearning 2>/dev/null || true
...
ip addr add fdff:965f:f02a:2::/127 dev m2mgw 2>/dev/null || true

# VRF c2m
ip link add vr.c2m type vrf table 2002027 2>/dev/null || true
ip link add vx.c2m type vxlan id 2002027 dev lo local 192.0.2.2 dstport 4789 nolearning 2>/dev/null || true
...
ip addr add fde9:9ec5:829e:2::/127 dev c2mgw 2>/dev/null || true

# Static routes to m2mgw/c2mgw
ip -4 route add 10.102.0.0/24 via inet6 fdff:965f:f02a:2::1 dev m2mgw vrf vr.m2m 2>/dev/null || true
ip -6 route add fda5:25c1:193c::/64 via inet6 fdff:965f:f02a:2::1 dev m2mgw vrf vr.m2m 2>/dev/null || true
ip -4 route add 10.102.1.0/24 via inet6 fde9:9ec5:829e:2::1 dev c2mgw vrf vr.c2m 2>/dev/null || true
ip -6 route add fda5:25c1:193d::/64 via inet6 fde9:9ec5:829e:2::1 dev c2mgw vrf vr.c2m 2>/dev/null || true

# NAT64 gateway
ip link set eth-nat64 master vr.mgmt 2>/dev/null || true
ip addr add fddf:64ff:9b00:2::/127 dev eth-nat64 2>/dev/null || true
ip link set eth-nat64 up 2>/dev/null || true
# Static routes to NAT64 in vr.mgmt
ip -6 route add fda5:25c1:193e::1/128 via fddf:64ff:9b00:2::1 dev eth-nat64 vrf vr.mgmt 2>/dev/null || true
ip -6 route add 64:ff9b::/96 via fddf:64ff:9b00:2::1 dev eth-nat64 vrf vr.mgmt 2>/dev/null || true
